...
Implementacja wzorca pozwala na obsługę żądań kierowanych do konkretnej strony - \textit{index.html}. Za~obsługę żądania odpowiedzialny jest servlet, który - po~wykonaniu operacji przetwarzania danych (użycie logiki aplikacji) - określa widok i~tworzy go, używając uzyskanych informacji.

\subsection{Szczegóły implementacji}
Implementacja wzorca składa się~z~dwóch beanów, które są~wykorzystane do~przetworzenia danych (\textit{session bean'y} - Bar i Barman), servletu oraz strony jsp (bar.jsp), która zostaje skompilowana na~serwerze przy użyciu dostarczonych przez servlet informacji.
